...
export const Technology = ({ technologies,onClosed }: Props) => {
  const categories: Category[] = ["Lenguajes", "Frameworks", "Herramientas"];

  return (
    <Card title="Habilidades Tecnologias" classNameTitle="bg-indigo-800 text-white" onClosed={()=>onClosed("technology")} >
      <div className="">
            ...
            <div key={category}>
              ...
              <div className="grid grid-cols-2 gap-2 pl-4">
                {items.map(({ name, icon, bgColor, textColor }) => (
                  <p
                    key={name}
                    className={`border p-2 rounded-md flex items-center gap-2 ${bgColor} ${textColor ?? ""}`}
                  >
                    {icon} {name}
                  </p>
                ))}
